...
than the funnel's contract (``std::init`` exempts defaulted copy/move
constructors, which initialize member-wise while writing no initializer).
Each check gates its diagnostics on
