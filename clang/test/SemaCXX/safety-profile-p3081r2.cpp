...
// RUN: %clang_cc1 -fsyntax-only -verify=applied -std=c++20 -fsafety-profile-apply=type -fsafety-profile-apply=bounds -fsafety-profile-apply=lifetime -fsafety-profile-apply=arithmetic -Wsafety-profile-type -Wsafety-profile-bounds -Wsafety-profile-lifetime -Wsafety-profile-arithmetic %s
...
//   profiles::enforce(P) or profiles::apply(P) had been written on an
//   empty-declaration may be defined in an implementation-defined
//   manner."
//
// Tested by -fsafety-profile-enforce=* and -fsafety-profile-apply=*
// flags on the RUN lines above.
// =====================================================================
...
//
// [dcl.attr.profile]: "At a program point Q, a profile P is applied if
//   it is not suppressed and: the profile attribute profiles::apply(P)
//   has been encountered"
//
...
//
// [dcl.attr.profile]: "if P is applied at Q, [the implementation] may
//   emit a diagnostic that C is discouraged."
// =====================================================================

void test_enforced_vs_applied_distinction() {
  int x;
...
// =====================================================================
// [dcl.attr.profile]: "enforcing or applying a profile does not change
//   the semantics of the program's code, such as the results of
//   overload resolution or whether special member functions are
//   deleted."
//
...
// applied-warning@-2 {{'const_cast' that casts away constness is discouraged}}
...
// applied-note@-2 {{in instantiation of default function argument}}
...
void test_suppress_single_statement() {
  ...
  // applied-warning@-2 {{narrowing 'static_cast'}}
...
void test_static_cast_narrowing() {
  ...
  // applied-warning@-2 {{narrowing 'static_cast' from 'double' to 'float'}}

  long long big = 1LL;
  float g = static_cast<float>(big);
  // enforced-error@-1 {{narrowing 'static_cast' from 'long long' to 'float'}}
  // applied-warning@-2 {{narrowing 'static_cast' from 'long long' to 'float'}}
...
void test_c_style_and_functional_casts() {
  ...
  // applied-warning@-2 {{narrowing 'static_cast' from 'double' to 'int'}}

  // Functional cast resolving to narrowing static_cast
  long long ll = 100;
  int j = int(ll);
  // enforced-error@-1 {{narrowing 'static_cast' from 'long long' to 'int'}}
  // applied-warning@-2 {{narrowing 'static_cast' from 'long long' to 'int'}}
}
...
void test_free() {
  ...
  // applied-warning@-2 {{calling 'free' is discouraged by profile std::lifetime}}
}
...
void test_narrowing_init_list() {
  ...
  // enforced-note@-2 {{insert an explicit cast}}
  // applied-error@-3 {{non-constant-expression cannot be narrowed from type 'long long' to 'bool'}}
  // applied-note@-4 {{insert an explicit cast}}
}
...
void test_arithmetic_overflow() {
  ...
  // applied-warning@-3 {{overflow in expression; result is}}
...
//
// [dcl.attr.profile]: "profiles::enforce or profiles::apply may be
//   applied only to empty-declaration"
...
//
// [dcl.attr.profile]: "Enforcing or applying profile std::strict is
//   equivalent to enforcing or applying all of profiles std::type,
//   std::bounds, and std::lifetime individually."
//   -> safety-profile-strict.cpp
//
// [dcl.attr.profile]: enforce/apply conflict
//   -> safety-profile-conflict.cpp
//
