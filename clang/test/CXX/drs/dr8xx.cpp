...
void g(int i) {
  ...
  //   since-cxx11-note@#dr873-lvalue-ref {{candidate function [with T = int &] has been explicitly deleted}}
  f(0); // calls f<int>(int&&)
  // since-cxx11-error@-1 {{call to deleted function 'f'}}
  //   since-cxx11-note@#dr873-rvalue-ref {{candidate function [with T = int] has been explicitly deleted}}
}
#endif
} // namespace dr873
