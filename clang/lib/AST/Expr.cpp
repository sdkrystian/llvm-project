...
MemberExpr::MemberExpr(Expr *Base, bool IsArrow, SourceLocation OperatorLoc,
  ...
  MemberExprBits.HasQualifier = QualifierLoc ? 1 : 0;
...
MemberExpr *MemberExpr::Create(
  ...
  std::size_t Size =
      totalSizeToAlloc<NestedNameSpecifierLoc, DeclAccessPair,
                       ASTTemplateKWAndArgsInfo, TemplateArgumentLoc>(
          QualifierLoc ? 1 : 0, HasFoundDecl ? 1 : 0,
          HasTemplateKWAndArgsInfo ? 1 : 0,
          TemplateArgs ? TemplateArgs->size() : 0);

  void *Mem = C.Allocate(Size, alignof(MemberExpr));
  MemberExpr *E = new (Mem) MemberExpr(Base, IsArrow, OperatorLoc, QualifierLoc,
                                       TemplateKWLoc, MemberDecl, FoundDecl,
                                       NameInfo, TemplateArgs, T, VK, OK, NOUR);
  return E;
}
