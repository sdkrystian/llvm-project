...
struct TemplateInstantiationArgumentCollecter
    ...
    if (const FunctionTemplateDecl *Primary = FD->getPrimaryTemplate()) {
      ...
      if (Innermost)
        ...
        AddOuterTemplateArguments(
            FD, FD->getTemplateSpecializationArgs()->asArray(),
            /*Final=*/false);

      if (FD->getTemplateSpecializationKind() == TSK_ExplicitSpecialization ||
          (FD->getFriendObjectKind() && !Primary->getFriendObjectKind()))
        return UseNextDecl(FD);

      // If this function was instantiated from a specialized member that is
      // a function template, we're done.
      if (Primary->getMostRecentDecl()->isMemberSpecialization())
        return Done();
  ...
  VisitClassTemplateSpecializationDecl(ClassTemplateSpecializationDecl *CTSD) {
    ...
    if (auto *CTPSD =
      ...
      if (CTPSD->getMostRecentDecl()->isMemberSpecialization())
        return Done();
    } else {
  ...
  VisitVarTemplateSpecializationDecl(VarTemplateSpecializationDecl *VTSD) {
    ...
    if (auto *VTPSD =
      ...
      if (VTPSD->getMostRecentDecl()->isMemberSpecialization())
        return Done();
    } else {
      auto *VTD = Specialized.get<VarTemplateDecl *>();
      if (VTD->getMostRecentDecl()->isMemberSpecialization())
        return Done();
    }
    return UseNextDecl(VTSD);
  }
