...
Value *ScalarExprEmitter::EmitScalarConversion(Value *Src, QualType SrcType,
  ...
  if (OrigSrcType->isFloatingType() && isa<llvm::IntegerType>(DstTy))
    CGF.EmitProfileRuntimeCheck(
        "std::core_ub", "float_cast_overflow",
        diag::trap_profile_float_cast_overflow, Loc, [&] {
...
Value *ScalarExprEmitter::EmitShl(const BinOpInfo &Ops) {
  ...
  if (!CGF.getLangOpts().OpenCL && !CGF.getLangOpts().HLSL &&
    ...
    if (CheckBase || !ConstantInRange)
      CGF.EmitProfileRuntimeCheck(
          "std::core_ub", "invalid_shift", diag::trap_profile_invalid_shift,
          Ops.E->getExprLoc(), [&]() -> llvm::Value * {
            bool RHSIsSigned = Ops.rhsHasSignedIntegerRepresentation();
            llvm::Value *WidthMinusOne =
                GetMaximumShiftAmount(Ops.LHS, Ops.RHS, RHSIsSigned);
